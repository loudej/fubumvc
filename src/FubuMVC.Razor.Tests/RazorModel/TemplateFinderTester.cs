...
namespace FubuMVC.Razor.Tests.RazorModel
{
    ...
    public class TemplateFinderTester : InteractionContext<TemplateFinder<Template>>
    {
        ...
        protected override void beforeEach()
        {
            ...
            _pak1 = new BottleInfo("Pak1");
            _pak2 = new BottleInfo("Pak2");
            ...
            _pak1.RegisterFolder(CommonBottleFiles.WebContentFolder, _pak1Path);
            _pak2.RegisterFolder(CommonBottleFiles.WebContentFolder, _pak2Path);
        ...
        {
            var items = ClassUnderTest.FindInHost().ToList();
            items.ShouldContain(x => x.FilePath == Path.Combine(_templatePath, "A3.cshtml"));
            items.ShouldContain(x => x.FilePath == Path.Combine(_templatePath, "A4.cshtml"));
            items.ShouldContain(x => x.FilePath == Path.Combine(_templatePath, "Views", "F1", "G1.cshtml"));
            items.ShouldContain(x => x.FilePath == Path.Combine(_templatePath, "Pak1", "Alpha", "Foxtrot", "Golf.cshtml"));
        }
